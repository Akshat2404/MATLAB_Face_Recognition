...
Facedetect = selectLabels(gTruth,'Eyes');
% 2. Creating Variable facedetect in which will store labels 'lips'

if isfolder(fullfile('EyesTrainingData'))    
cd EyesTrainingData
else 
mkdir EyesTrainingData
end
addpath('EyesTrainingData');


% 3. if else condition, if means 'if full name EyesTrainingData exist ,locate that file ' else here if EyesTrainingData Files doesnt exits Make one and 
%         it to the MATLAB Path

trainingData = objectDetectorTrainingData(Facedetect,'SamplingFactor',1,'writeLocation','EyesTrainingData');
...
Edetector = trainACFObjectDetector(trainingData,'NumStages',20);
